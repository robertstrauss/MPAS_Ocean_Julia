...
function heatMapMesh(myMPAS_O, phi; fig="new", ax="new", cbar="new", cmap="seismic", cMin="auto", cMax="auto")
    ...
    for iCell in 1:myMPAS_O.nCells
        ...
        cellpatch = mplpatches.Polygon(vertices, closed=true)
...
function signedEdgeHeatMapMesh(myMPAS_O, phi; fig="new", ax="new", cbar="new", cmap="Greys", cMin=0, cMax="auto", linewidth=1)
    if fig == "new"
        fig = figure()
    end
    if ax == "new"
    ...
    for iCell in 1:myMPAS_O.nCells
        for i in 1:myMPAS_O.nEdgesOnCell[iCell]
            ...
            truephi = phi[iEdge] * myMPAS_O.edgeSignOnCell[iCell, i]
            intoCell = truephi > 0 ? myMPAS_O.cellsOnEdge[1, iEdge] : myMPAS_O.cellsOnEdge[2, iEdge]
            
            vertices = [
                myMPAS_O.xVertex[vertexIndices[1]]      myMPAS_O.yVertex[vertexIndices[1]];
                myMPAS_O.xVertex[vertexIndices[2]]      myMPAS_O.yVertex[vertexIndices[2]];
                myMPAS_O.xCell[intoCell]                   myMPAS_O.yCell[intoCell]
            ]
            
#             # bring the edge line slightly closer to the center of the cell, so its clear which cell its on
#             nudge = 0.1 * linewidth # / myMPAS_O.dcEdge[iEdge]
#             # basically linear combination of vertex position and cell center position, with weights 1 - nudge and nudge
#             vertices .*= 1.0 - nudge
#             vertices[:,1] .+= nudge * myMPAS_O.xCell[iCell]
#             vertices[:,2] .+= nudge * myMPAS_O.yCell[iCell]

            # edge cases
            for v = 1:length(vertices)-1
                if maximum(abs.(vertices[v] - vertices[v+1])) > 23*myMPAS_O.gridSpacingMagnitude
                    vertices .= 0
                end
            end
#             if abs(vertices[1,1] - vertices[2,1]) > 1.01*myMPAS_O.gridSpacingMagnitude
...
#             end
            ...
            edgepatch = mplpatches.Polygon(vertices, closed=false, color=m.to_rgba(phi[iEdge] * myMPAS_O.edgeSignOnCell[intoCell, i]), linewidth=linewidth)
            append!(patches, [edgepatch])
        end
    end
...
function vectorNormalEdgeHeatMapMesh(myMPAS_O, phi; fig="new", ax="new", cbar="new", cmap="seismic", cMin="auto", cMax="auto", linewidth=1)
    if fig == "new"
        fig = figure()
    end
    if ax == "new"
...
function streamVectorEdgeHeatMapMesh(myMPAS_O, phi; fig="new", ax="new", cbar="new", cmap="seismic", cMin="auto", cMax="auto", linewidth=1)
    if fig == "new"
        fig = figure()
    end
    if ax == "new"
...
#     ax.add_collection(localPatches)
    ...
    X = collect(range(0, myMPAS_O.lX, length=10))
    Y = collect(range(0, myMPAS_O.lY, length=00))
    ...
    ax.streamplot(X, Y, phiXUniformBinned, phiYUniformBinned, color="k")
