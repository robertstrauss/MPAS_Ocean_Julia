...
if rank == root
	println("running with \$(commsize) cores and \$(Threads.nthreads()) threads per core for a total of \$(Threads.nthreads()*commsize) processes")
end
"""
#
...
"""
if rank == root
	println("dividing ocean into \$(nXChunks)x\$(nXChunks) among \$(commsize) nodes")
end
...
open("/global/cscratch1/sd/rstrauss/runs/fortrancomparison/divided1000x1000ocean_\$(rank).txt", "w") do io
...
kelvinWaveExactSolution!(mpasOcean, 0)
...
"""
if rank == root
	println("cells per chunk: ", length(myOcean.sshCurrent))
	println("procs per chunk: ", Threads.nthreads())
	println("cells per proc: ", length(myOcean.sshCurrent)/Threads.nthreads())
end
"""
# if rank == root
